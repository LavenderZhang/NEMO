//#################################################################\\
//# Modify the coordinates send as argument to UIWindow::UIWindow #\\
//# for Cash Shop Button to the user specified ones               #\\
//#################################################################\\

function MoveCashShopIcon()
{
    //Step 1.1 - Find the XCoord calculation pattern
    var code =
        " 81 EA BB 00 00 00" //SUB EDX, 0BB
    +   " 52"                //PUSH EDX
    ;
    var tgtReg = 2;
    var offset = Exe.FindHex(code);

    if (offset === -1)
    {
        code = code.replace("81 EA", "2D");//change EDX to EAX
        code = code.replace("52", "50");   //in both instructions
        tgtReg = 0;
        offset = Exe.FindHex(code);
    }
    if (offset === -1)
    {
        code = code.replace("50", "6A 10 50");//PUSH 10 before PUSH EAX
        offset = Exe.FindHex(code);

        //Step 1.2 - If this pattern matches then put the
        //           PUSH 10 first and update offset by 2
        if (offset !== -1)
        {
            Exe.ReplaceHex(offset, "6A 10");
            offset += 2;
        }
    }
    if (offset === -1)
        return "Failed in Step 1 - Coord calculation missing";

    //Step 1.2 - Accomodate for extra bytes by NOPing those
    if (tgtReg === 2) //EDX
    {
        Exe.ReplaceHex(offset, "90");
        offset++;
    }

    //Step 1.3 - Find the pattern where the Screen Size is picked up (Width is at 0x24, Height is at 0x28) - We need the address of g_ScreenStats
    code =
        " 8B 0D ?? ?? ?? 00" //MOV ECX, DWORD PTR DS:[g_ScreenStats]
    +   " 8B"                //MOV reg32_A, DWORD PTR DS:[reg32_B+const]
    ;
    var offset2 = Exe.FindHex(code, offset - 0x18, offset);

    if (offset2 === -1)
    {
        code = code.replace("8B 0D", "A1"); //swap ECX with EAX
        offset2 = Exe.FindHex(code, offset - 0x18, offset);
    }
    if (offset2 === -1)
        return "Failed in Step 1 - Screen Size retrieval missing";

    //Step 1.4 - Extract the g_ScreenStats
    var g_ScreenStats = Exe.GetHex(offset2 + code.byteCount() - 5, 4);

    //Step 2.1 - Get User Coords
    var xCoord = Exe.GetUserInput("$cashShopX", I_INT16, "Number Input", "Enter new X coordinate:", -0xBB, -0x8000); //Dialog will auto correct maximum values
    var yCoord = Exe.GetUserInput("$cashShopY", I_INT16, "Number Input", "Enter new Y coordinate:", 0x10, -0x8000);

    if (xCoord === -0xBB && yCoord === 0x10)
        return "Patch Cancelled - New coordinate is same as old";

    //Step 2.2 - Prep code to insert based on the sign of each coordinate (negative values are relative to width and height respectively)
    code = "";

    if (yCoord < 0)
    {
        code +=
            " 8B 0D" + g_ScreenStats    //MOV ECX, DWORD PTR DS:[g_ScreenStats]
        +   " 8B 49 28"                 //MOV ECX, DWORD PTR DS:[ECX+28]
        +   " 81 E9" + Num2Hex(-yCoord) //SUB ECX, -yCoord
        ;
    }
    else
    {
        code += " B9" + Num2Hex(yCoord);//MOV ECX, yCoord
    }
    code += " 89 4C 24 04"; //MOV DWORD PTR DS:[ESP+4], ECX

    if (xCoord < 0)
    {
        code +=
            " 8B 0D" + g_ScreenStats    //MOV ECX, DWORD PTR DS:[g_ScreenStats]
        +   " 8B 49 24"                 //MOV ECX, DWORD PTR DS:[ECX+24]
        +   " 81 E9" + Num2Hex(-xCoord) //SUB ECX, -xCoord
        ;
    }
    else
    {
        code += " B9" + Num2Hex(xCoord);//MOV ECX, xCoord
    }
    code +=
        " 89" + Num2Hex(0xC8 | tgtReg, 1) //MOV tgtReg, ECX
    +   " C3"                             //RETN
    ;

    //Step 2.3 - Find Free space for insertion
    var free = Exe.FindSpace(code.byteCount());
    if (free === -1)
        return "Failed in Step 2 - Not enough free space";

    //Step 3.1 - Insert the code at free space
    Exe.InsertHex(free, code, code.byteCount());

    //Step 3.2 - Change the 0xBB subtraction with a call to our code
    Exe.ReplaceHex(offset, "E8" + Num2Hex(Exe.Real2Virl(free, DIFF) - Exe.Real2Virl(offset + 5, CODE)));
    return true;
}

//=====================================================//
// Only Enable for Clients that actually have the icon //
//=====================================================//
function MoveCashShopIcon_()
{
    return (Exe.FindString("NC_CashShop", REAL) !== -1);
}