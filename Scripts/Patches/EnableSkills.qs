...
function EnableSkills(oldPatn, newPatn, patchID, funcName, isPlayerFn)
{
    ...
    if (isPlayerFn)
    {
        ...
        +   GenLuaCaller(fnBegin + 4, funcName, Exe.Real2Virl(fnBegin + 0x80, CODE), "d>d", " 50")
        +   " C3"           //RETN ; AL is already set
        ;
        
        //Step 3.2 - Overwrite function with our code
        Exe.ReplaceHex(fnBegin, code);

        //Step 3.3 - Add the function Names after the codes
        Exe.ReplaceString(fnBegin + 0x80, funcName);
    }
    else
    {
        //Step 4.1 - Find Free space for insertion considering max size
        var free = Exe.FindSpace(funcName.length + 0x3D + 1);//for RETN
        if (free === -1)
        ...
        code =
            Ascii2Hex(funcName)
        +   GenLuaCaller(free, funcName, Exe.Real2Virl(fnBegin + 0x10, CODE), "d>d", " 52")
        +   " C3" //RETN
        ;
        ...
        +   " E8" + MakeVar(1)  //CALL ourFunc
        +   " 5A"               //POP EDX
        +   " C2 04 00"         //RETN 4
        ;
        code = SetValue(code, 1, Exe.Real2Virl(free + funcName.length, DIFF) - Exe.Real2Virl(fnBegin + 10));
