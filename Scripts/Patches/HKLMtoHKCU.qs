//#################################################################\\
//# Change all HKEY_LOCAL_MACHINE parameters to HKEY_CURRENT_USER #\\
//#################################################################\\

function HKLMtoHKCU()
{
    //Step 1 - Find all occurrences of HKEY_LOCAL_MACHINE (0x80000002)
    var offsets = Exe.FindAllHex("68 02 00 00 80");
    if (offsets.length == 0)
    ...
    {
        if (Exe.GetInt8(offsets[i] + 5) !== 0x3B)
            Exe.ReplaceInt8(offsets[i] + 1, 0x01);
    }
    return true;
}