...
//###############################################################################\\
//# Modify the Max Height from Ground (FAR_DIST) to accomodate for larger zoom  #\\
//###############################################################################\\
...
function IncreaseZoomOut(newValue)
{
    ...
    var offset = Exe.FindHex(code, Exe.GetRealOffset(DATA), Exe.GetSize());//Its not there in code section - so we use the generic find
    if (offset === -1)
        return "Failed in Step 1";

    //Step 2 - Modify with the value supplied - Current value is 400.0
    Exe.ReplaceHex(offset + 6, newValue); //newvalue is actually just the higher 2 bytes of what is required, since lower 2 bytes are 0
    return true;
}